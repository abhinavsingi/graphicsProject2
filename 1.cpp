...
float rot_angle = 0,rot_angle1 = 0,rot_angle2 = 0;
float above = 0,fact = 1;
...
enum
{
	Distant,Outside,Inside,Driver,Helicopter,Balloon,roofTop,Revolve,Overhead,Wheel,Above,Light
} view = Distant;
...
class steering
{
	public:
		...
		glPushMatrix();
			int rad = 2;
			glTranslatef(x,y,0);
			glRotatef(rot_angle2,0,0,1);
			glLineWidth(4);
			
			glBegin(GL_LINE_LOOP);
			for(int i=0;i<360;i++)
			{
				glVertex2f(rad*cos(i*(3.14/180)),rad*sin(i*(3.14/180)));
			}
			glEnd();
			glBegin(GL_LINES);
			glVertex2f(rad*cos(90*(3.14/180)),rad*sin(90*(3.14/180)));
			glVertex2f(rad*cos(270*(3.14/180)),rad*sin(270*(3.14/180)));
//			glVertex2f(rad*cos(180*(3.14/180)),rad*sin(180*(3.14/180)));
//			glVertex2f(rad*cos(0*(3.14/180)),rad*sin(0*(3.14/180)));
			glEnd();
		glPopMatrix();
		}
};
...
//	glutSolidCube(15.0f);
	...
	glPushMatrix();				//overhead box
		glColor3f(1,1,1);
		glTranslatef(-1,7,8);
		glRotatef(90,0,1,0);
		glRotatef(90,1,0,0);
		gluCylinder(pointer, 1.0, 1.0, 15.0, 10, 12);
	glPopMatrix();

	glPushMatrix();
		glTranslatef(0,above,10);
		glColor3f(1,0,0);
		glutSolidCube(4);
		glPushMatrix();
			glColor3f(0,1,0);
			...
			glRotatef(90,0,1,0);
			glRotatef(90,1,0,0);
			glRotatef(rot_angle1,1,0,0);
			gluCylinder(pointer,1,1,3,10,12);
//			glutSolidSphere(1,10,10);
	...
	glPopMatrix();
	glPushMatrix();					//steering wheel
		glTranslatef(5,0,0);
		glRotatef(90,0,1,0);
								// roatating the wheel when left or right key is pressed
		if(flag3 ==1)
			glRotatef(carPos + 30, 0,0,-1);
		if(flag4 == 1)
			glRotatef(carPos - 30, 0,0,-1);
		obj1[1].x = 0;
		obj1[0].y = 0;
...
//			glRotatef(90,1,0,0);
//			glRotatef(rot_angle1,1,0,0);
			glRotatef(rot_angle1, 0, 1, 0);
			gluLookAt(2.0, above, height,
				12.0, above, 2.0,
				0.0, 0.0, 1.0 );
			drawCar();
			glRotatef(180/PI * carPos, 0.0, 0.0, 1.0);
...
void display(void)
{
	...
	glLoadIdentity();
//	if(flag3 == 1)
//		rot_angle2 = ((int)rot_angle2 - 2)%10;
//	rot_angle1 = ((int)rot_angle1 -3)%360;
	rot_angle1 = (rot_angle1 +0.3);
	if(flag1 == 1 || flag2 == 1)
		rot_angle=((int)rot_angle-3)%360;
	viewpoint();
